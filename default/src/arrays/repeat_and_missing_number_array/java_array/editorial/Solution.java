package arrays.repeat_and_missing_number_array.java_array.editorial;
    ...
    public static int[] repeatedNumber(final int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (arr[targetIndex] > 0) {
                ...
                //it means that arr[targetIndex] is already < 0
                // => we already faced with this targetIndex
                // => we already faced with this arr[i] value
                // => we find repeated value!
                result[0] = Math.abs(arr[i]);
            }
        }

        //EXAMPLE: {-5, 3, -1, -4, -3};
        //now all non-repeated values are < 0
        //except the value that has index (let's name it MissedIndex)
        //since we did *-1 for elements finding them via index -> we missed the index related to missed value

        //EXAMPLE: {-5, 3, -1, -4, -3}; the only positive element 3 has index 1 => the answer is 1+1 = 2
        for (int i = 0; i < n; i++) {
            if (arr[i] > 0) {
                result[1] = i + 1;
            }
